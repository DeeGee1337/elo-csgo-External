...
namespace rn {
	...
	private:
		...
		)
		{
			...
			auto fixed_dir = fix_seperators(directory);
			auto fixed_name = fix_seperators(map_name);

			file_path = fixed_dir + "/" + fixed_name;
	...
	public:
		...
		bool is_visible(const vec3& origin, const vec3& destination)
		{
		...
		)
		{
			if (!_planes.empty() && out) {
				...
				ray_cast_node(0, 0.f, 0.f, origin, final, out); 
