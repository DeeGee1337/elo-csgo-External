...
#else
#define XorString(str) xorstr_(str)
#endif
#ifdef _MSC_VER
...
namespace jm {
	...
	namespace detail {
		...
		template<size_t I, size_t M, class T>
		...
		struct tstring_ {
			...
			constexpr static size_t size_in_bytes() noexcept
			{
				return (size + 1) * sizeof(value_type);
			}
		};

		constexpr static void hash_single(uint32_t& value, char c) noexcept
		{
		...
		struct string_storage {
			uint64_t storage[buffer_size<T>()];
	...
	struct xor_string {
		alignas(detail::buffer_align<T>())
			XORSTR_VOLATILE uint64_t _storage[detail::buffer_size<T>()];
		...
		XORSTR_FORCEINLINE void _crypt() noexcept
		{
			if constexpr (detail::buffer_size<T>() > N) {
				...
				if constexpr ((detail::buffer_size<T>() - N) >= 4) {
					...
					// = { ... }
					alignas(32) XORSTR_CLANG_VOLATILE uint64_t keys[4];
...
#endif
				{
					alignas(16) XORSTR_VOLATILE uint64_t keys[2];
					keys[0] = detail::key8<N + 0>();
					keys[1] = detail::key8<N + 1>();
		...
		template<size_t N>
		XORSTR_FORCEINLINE constexpr static uint64_t _at()
		{
			// forces compile time evaluation of storage for access
			constexpr uint64_t val = detail::string_storage<T>{}.storage[N];
			return val;
		}
