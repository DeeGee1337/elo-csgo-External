...
)
{
    ...
    _render_target->SetAntialiasMode(D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
...
)
{
    if (_render_target)
    {
        ...
        _render_target->DrawEllipse(&ellipse, _brush, 1.0f);
    }
}
...
)
{
    ...
    if( hex != _last_color ) {
        FLOAT alpha = col.a() / 255.0f;
        _brush->SetColor( D2D1::ColorF( hex, alpha) );
        _last_color = hex;
    }
}
